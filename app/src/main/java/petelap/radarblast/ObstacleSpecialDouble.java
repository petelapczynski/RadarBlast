...
public class ObstacleSpecialDouble extends ObstacleSpecials implements IGameObjectSpecial {

    public ObstacleSpecialDouble(float startX, float startY, float radius, int color) {
        this.radius = radius;
        ...
        InGameArea = true;
        pop = false;
        image = BitmapFactory.decodeResource(Constants.CONTEXT.getResources(),R.drawable.spec_double);
    ...
    public static IGameObjectSpecial GetInstance() {
        return new ObstacleSpecialDouble(0,0,85, Color.WHITE);
    }
    ...
    public void draw(Canvas canvas) {
        ...
        matrix.setRotate(spin, image.getWidth()/2, image.getHeight()/2);
        matrix.postTranslate(center.x - image.getWidth()/2 , center.y - image.getHeight()/2);
